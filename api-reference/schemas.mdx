...
interface Note {
  ...
  audio_name: string;            // Nome do arquivo de áudio
  audio_url: string;             // URL para download do áudio
  transcription_status: "pending" | "processing" | "completed" | "failed"; // Status da transcrição
  transcription_url?: string;    // URL para download da transcrição (opcional)
