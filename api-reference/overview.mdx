...
- **Requisições por minuto**: Conforme seu plano
- **Requisições por hora**: Conforme seu plano
...
|----------|--------|-----------|
| `/notes` | POST | Criar nova nota com upload de áudio |
| `/notes` | GET | Listar notas com filtros |
| `/notes/{id}` | GET | Obter detalhes de uma nota específica |
| `/notes/{id}/status` | GET | Obter status de transcrição de uma nota |
...
<Info>
**Dica**: Para verificar apenas o status de uma nota específica sem buscar todos os dados, use o endpoint [`/notes/{noteId}/status`](/api-reference/notes/status).
</Info>
